...
CupTaster - бесплатное приложения для бариста, продавцов и кофеманов, которые занимаются профессиональной дегустацией кофе.
...
- [Содержание папок и файлов (краткое описание содержания каждой папки и файла)](#Содержание-папок-и-файлов)
...
Папка со всеми связанными с каппингом файлами.
...
Папка со всеми связанными с образцом файлами.
...
Папка со всеми связанными с настройкой приложения файлами.

* [Settings](CupTaster/Settings/Settings.swift) - form с отобржаением всех настроек в приложении
* [Cupping Forms](CupTaster/Settings/S%20Cupping%20Forms.swift) - возможность *выбора по умолчанию / добавления новой / удаления существующей* формы для каппинга. Этот view отображается как в настройках, так и в sheet в onboarding.
* [General Info](CupTaster/Settings/S%20General%20Info.swift) - экран в возможностью добавить шаблоны general info

## [Cupping Forms](CupTaster/Cupping%20Forms)

Формы для каппинга в формате JSON а так же способы взаимодействия с ними.

* [Cupping Forms Manager](CupTaster/Cupping%20Forms/CFManager.swift) - тут происходит вся магия добавления формы из JSON в объект базы данных. Для этого достаточно добавить в массив allCFModels новый объект типа CFModel, инициализация которого требует только название и версию формы. В этом же менеджере добавлены функции: получения формы по умолчанию, установки формы по умолчанию, проверка актуальности версий форм для каппинга, проверка наличия подсказок в конкретной форме для каппинга или в форме по умолчанию. 
* [Cupping Form Model](CupTaster/Cupping%20Forms/CFModel.swift) - приватный класс менеджера форм для каппинга, который добавляет форму для каппинга в базу данных. Там же прописана функция получения добавленной формы для каппинга по названию.
* [SCA Cupping Form (JSON)](CupTaster/Cupping%20Forms/SCA.json) - JSON формы для каппинга от Specialty Coffee Association (получено разрешение на использование)
* [SCI Cupping Form (JSON)](CupTaster/Cupping%20Forms/SCI.json) - JSON формы для каппинга от Sustainable Coffee Institute (получено разрешение на использование)
* [CoE Cupping Form (JSON)](CupTaster/Cupping%20Forms/CoE.json) - JSON формы для каппинга от Cup of Excellence (отправлено письмо с запросом на использование)

## [Helpers](CupTaster/Helpers)

Дополнительные файлы, которые нельзя отнести к той или иной папке.

* [Stopwatch](CupTaster/Helpers/Stopwatch.swift) - секундомер для того, чтобы пробовать кофе на разных стадиях остывания, сравнивать между собой эти вкусы и понимать в какой момент вкус раскрывается больше всего. Запустить можно почти на всех экранах вкладки Cupping.
* [Hide Keyboard](CupTaster/Helpers/Hide%20Keyboard.swift) - extension с функцией, которая срабатывает как триггер для жеста drag и скрытвает клавиатуру.
* [AppStorage extensions](CupTaster/Helpers/AppStorage%20extensions.swift) - extensions для разных типов данных, позволяюший сохранять их в UserDefaults
* [Half Sheet](CupTaster/Helpers/Half%20Sheet.swift) - расширение для отображения лишь половины sheet. используется для отображения первоначальной настройки каппинга.
* [Email Helper](CupTaster/Helpers/Email%20Helper.swift) - класс с функцией отправки письма по эл. почте прямо из приложения.
* [Number To Digits](CupTaster/Helpers/NumToDigits.swift) - перевод числа в массив с числами *(123 -> [1, 2, 3])*.

## [CoreData](CupTaster/CoreData)
  
Папка со всеми сущностями базы данных, а так же самой CoreData (xcdatamodeld) базой данных.

* [Entities](CupTaster/CoreData/Entities) - папка с обычными сущностями базы данных.
* [Codable Entities](CupTaster/CoreData/Codable%20Entities) - папка с сущностями, которые можно декодировать из JSON.

## [Resources](CupTaster/Resources)
  
Папка с дополнительными файлами, необходимыми для корректной работы приложения.
